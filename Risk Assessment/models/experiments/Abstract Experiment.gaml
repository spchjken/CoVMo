...
experiment AbstractExp virtual: true {
	...
	parameter "Xem xếp hạng" category: "Hiển thị" var: show_ranking <- false;
...
//			overlay position: {100, 0} size: {270 #px, 420 #px} transparency: 0.0  {
//				if (show_ranking) {
			...
			graphics "Info" position: {0, 0, 0.006} {
				...
				if (#zoom >= 1.5) {
					if (under_mouse_agent != nil) {
						...
						draw str at: {info_target.x - (length(str) * 20), info_target.y} empty: false perspective: false font: info border: #black color: text_color;
					}
